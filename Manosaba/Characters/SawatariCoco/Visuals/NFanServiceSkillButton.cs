...
public sealed partial class NFanServiceSkillButton : Control
{
    ...
    {
        _cachedActivateLabel = new LocString("powers", "MANOSABA-FAN_SERVICE_SKILL_POWER.activateLabel").GetFormattedText();
        _hoverTip = new HoverTip(
            new LocString("powers", "MANOSABA-FAN_SERVICE_SKILL_POWER.title"),
            new LocString("powers", "MANOSABA-FAN_SERVICE_SKILL_POWER.description"));
    ...
    {
        if (_player == null || !FanServiceSkillActivation.CanActivate(_player))
        {
            return;
        }
        ...
        {
            FanServiceSkillActivation.TryEnqueue(_player);
            Refresh(force: true);
            AcceptEvent();
        }
    }
    ...
    private void RestoreIdleVisuals()
    {
        ...
        bool canClick = _player != null && FanServiceSkillActivation.CanActivate(_player);
    ...
    private void Refresh(bool force)
    {
        ...
        FanServiceSkillPower? skillPower = _player.Creature?.GetPower<FanServiceSkillPower>();
        ...
        bool canClick = FanServiceSkillActivation.CanActivate(_player);
...
public static class FanServiceSkillButtonUi
{
    private const string ButtonNodeName = "FanServiceSkillButtonInjected";

    public static void EnsureShown(Player player)
    {
        NCombatUi? combatUi = NCombatRoom.Instance?.Ui;
        if (combatUi == null || player.Creature?.GetPower<FanServiceSkillPower>() == null)
        {
            return;
        }
        ...
        NFanServiceSkillButton? existing = FindButton(combatUi);
        if (existing != null)
        {
        ...
        NFanServiceSkillButton button = new();
    ...
    public static void RefreshIfPresent(NCombatUi combatUi, ICombatState state)
    {
        ...
        if (me?.Creature?.GetPower<FanServiceSkillPower>() == null)
        {
            RemoveFrom(combatUi);
            return;
        }
    ...
    private static NFanServiceSkillButton? FindButton(NCombatUi combatUi)
    {
        Node? node = combatUi.GetNodeOrNull(ButtonNodeName);
        if (node is NFanServiceSkillButton button)
        {
            return button;
        }
        ...
        {
            if (child is NFanServiceSkillButton match)
            {
                return match;
            }
        }
