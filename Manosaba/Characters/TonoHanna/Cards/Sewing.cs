...
public class Sewing : PathCustomCardModel
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        List<CardModel> puppetsInHand = PileType.Hand.GetPile(Owner).Cards
            .Where(c => c.Tags.Contains(ManosabaCardTags.Puppet))
            .ToList();
        if (puppetsInHand.Count == 0)
            return;

        if (IsUpgraded)
        {
            foreach (CardModel c in puppetsInHand)
                CardCmd.Upgrade(c);
            return;
        }

        var prefs = new CardSelectorPrefs(new LocString("cards", "MANOSABA-SEWING.selectionScreenPrompt"), 1);
        IEnumerable<CardModel> picked = await CardSelectCmd.FromHand(choiceContext, Owner, prefs, c => c.Tags.Contains(ManosabaCardTags.Puppet), this);
